...
%     nexttile([2,6]);
%     fig = pressure_mode_plot_3d_scatter();
...
%     nexttile(t,[1,6]);
    ...
    nexttile(t,3,[2,2]);
    [C,h] = contourf(X_plot_side, Z_plot_side, transpose(reshape(P(:,1,:), ...
        numel(x_vec), numel(z_vec))), v);
    set(h, 'edgecolor','none');
    hold on
    [C,h] = contour(X_plot_side, Z_plot_side, transpose(reshape(P(:,1,:), ...
        numel(x_vec), numel(z_vec))), v2, 'k');
    clabel(C,h,v2,'FontSize',5,'labelspacing', 1000)
    caxis(c);
    title('Side View');
    xlabel('x');
    ...
    innerTitle = 'Calculated Pressure Distribution';
    annotation('textbox', [0.21, 0.46, 0.6, 0.1], 'String', innerTitle,...
        'FontSize', 14, 'FontWeight', 'bold', 'HorizontalAlignment',...
        'center', 'LineStyle', 'none');
    innerTitle = 'Simulated Pressure Distribution';
    annotation('textbox', [0.21, 0.89, 0.6, 0.1], 'String', innerTitle,...
        'FontSize', 14, 'FontWeight', 'bold', 'HorizontalAlignment',...
        'center', 'LineStyle', 'none');
    
    pressure_mode_plot_3d_scatter(t, n_x, n_y, n_z, calc_average);
    ...
    exportgraphics(fig, ['../Testdata/modes_results/mode_f',num2str(f),'Hz.pdf'],'ContentType','image')
...
%     pbaspect([L_x L_z, 1]);
% 
%     exportgraphics(f1, ['../Testdata/modes_results/sim_mode_f',num2str(f),'Hz.pdf'],'ContentType','image')
%     exportgraphics(f2, ['../Testdata/modes_results/sim_mode_f',num2str(f),'Hz_top.pdf'],'ContentType','image')
%     exportgraphics(f3, ['../Testdata/modes_results/sim_mode_f',num2str(f),'Hz_side.pdf'],'ContentType','image')
