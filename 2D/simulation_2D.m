...
function simulation_2D()
    ...
    % SBP-SAT
    D_x = c^2*D2_x + c^2*HI_x*e_lx'*d1_lx*eye(m_x) - c^2*HI_x*e_rx'*d1_rx*eye(m_x);

    % Get D2 operator - y
    [~, HI_y, ~, D2_y, e_ly, e_ry, d1_ly, d1_ry] = sbp_cent_6th(m_y, h_y);
    % SBP-SAT
    D_y = c^2*D2_y + c^2*HI_y*e_ly'*d1_ly*eye(m_y) - c^2*HI_y*e_ry'*d1_ry*eye(m_y);
